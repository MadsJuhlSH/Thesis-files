...
def root_plot(m_ee, distribution, fit, mZmin: int, mZmax: int, title: str,  save_dir: str, DoF: int=100, nParam: int=3):
    # Initiate mZ
    ...
    ### chi2
    DoF = DoF-nParam # 3 fitting parameters
    chi2_fit = ZeeWenu_mZ_Frame.chiSquare()*DoF
    proba = 1-chi2.cdf(chi2_fit, DoF) # 100 bins and 3 variables floating

    fit.paramOn(ZeeWenu_mZ_Frame, ROOT.RooFit.Label(f'$\chi^2$={round(chi2_fit,2)}, p={round(proba,2)}, DoF={DoF}'),
    ROOT.RooFit.Layout(0.55,0.90,0.90), ROOT.RooFit.ShowConstants());
    ZeeWenu_mZ_Frame.getAttText().SetTextSize(0.03);
    ...
    gPad.Modified();
    gPad.Update();
    c_mZ.Print(f'./{save_dir}/{title}.pdf') # Save file. Remember that the folder needs to exist
#
    return mc_ZeeWenu_mZ_resid, chi2_fit

# def root_plot2(m_ee, distribution, fit, fit2, mZmin: int, mZmax: int, title: str,  save_dir: str, DoF: int=100, nParam: int=3):
#     # Initiate mZ
#     m_ee.setRange("MC_mZfit_range", mZmin, mZmax);
#     c_mZ = ROOT.TCanvas("c_mZ", "", 0, 0, 1000, 1000); #Make canvas
#     pad_mZdist = ROOT.TPad("pad_mZdist", "pad_mZdist", 0.0, 0.35, 1.0, 1.0);
...
#     distribution.plotOn(ZeeWenu_mZ_Frame, RooFit.Name(str(title)), RooFit.LineColor(2), RooFit.MarkerColor(2));
#     fit.plotOn(ZeeWenu_mZ_Frame, RooFit.Range("MC_mZfit_range"), RooFit.LineColor(2), RooFit.Name(str(title))); #Rød
#     fit2.plotOn(ZeeWenu_mZ_Frame, RooFit.Range("MC_mZfit_range"), RooFit.LineColor(1), RooFit.Name("Bkg"));
#
#     ### chi2
#     DoF = DoF-nParam # 3 fitting parameters
#     chi2_fit = ZeeWenu_mZ_Frame.chiSquare()*DoF
...
#     ROOT.RooFit.Layout(0.55,0.90,0.90), ROOT.RooFit.ShowConstants());
#     ZeeWenu_mZ_Frame.getAttText().SetTextSize(0.03);
#     fit2.paramOn(ZeeWenu_mZ_Frame, ROOT.RooFit.Label(f'$\chi^2$={round(chi2_fit,2)}, p={round(proba,2)}, DoF={DoF}'),
#     ROOT.RooFit.Layout(0.1,0.40,0.90), ROOT.RooFit.ShowConstants());
#     ZeeWenu_mZ_Frame.getAttText().SetTextSize(0.03);
#     mc_ZeeWenu_mZ_resid = ZeeWenu_mZ_Frame.pullHist(); #pullHist residHist
#     mc_ZeeWenu_mZ_resid.GetXaxis().SetRangeUser(mZmin, mZmax);
#     mc_ZeeWenu_mZ_resid.SetLineColor(2);
#     mc_ZeeWenu_mZ_resid.SetMarkerColor(2);
#     fit = ZeeWenu_mZ_Frame.findObject("LH fit");
#     fit2 = ZeeWenu_mZ_Frame.findObject("LH fit");
#     mc_ZeeWenu_mZ_resid.Draw('same');
#     legend.AddEntry( fit , str(title), "lep" );
#     legend.SetFillStyle(0);
#     ZeeWenu_mZ_Frame.Draw(); #50
#     #ZeeWenu_mZ_Frame.SetAxisRange(10**0,10**3,"Y");
#     legend.Draw('same');
#     c_mZ.cd();
#
#     # Ratio plot
...
def PeakFit_likelihood(Likelihood_cut: pd.DataFrame, mass_energy: pd.DataFrame, fig_title, save_dir, plots = True, constant_mean = True, constant_width = True, classifier_name = 'Likelihood', CB = True, Gauss = False, bkg_comb = True, bkg_exp = False, bkg_cheb = False):
    print('Starting fit...')
    matplotlib.use('Agg')
    ...
    DoF= np.array([h_mZ_all.GetBinContent(i) for i in range(NbinsZmass)])
    DoF = np.sum(DoF != 0)
    ...
    if plots: # Plots the signal using the function "root_plot" defined above
        mc_Zee_signal, chi2_signal = root_plot(m_ee=m_ee, distribution=mc_Zee_mZ, fit=func_BWxCB,
                  mZmin=mZmin, mZmax=mZmax, title=f'signal for cut {fig_title}', save_dir = save_dir, DoF=DoF, nParam=nParam_sig);
#cut {fig_title}
    # =============================================================================
    #    background
    # =============================================================================
    ...
    #if True:
    m_ee.setRange("MC_mZfit_range_SB1", 60, 75); # Set range for fit as defined in the beginning
    m_ee.setRange("MC_mZfit_range_SB2", 110, 130); # Set range for fit as defined in the beginning
    ...
    DoF= np.array([h_mZWenu_all.GetBinContent(i) for i in range(NbinsZmass)])
    DoF = np.sum(DoF != 0)

    # Create the lin + exponential fit
    lam = RooRealVar("lambda", "Exponent", -5.0, -0.04, 0.0);# -5.0, 0.0);
    ...
    m_ee.setRange("high", 110, 120);

    # Adding exponential and Chebychev if comb:
    if bkg_comb:
        func_ExpLin_mZ_unextended = ROOT.RooAddPdf("func_ExpLin_mZ_unextended", "Exponential and Linear PDF", RooArgList(func_Cpol2, func_expo), RooArgList(f_exp_mZ)); #
    elif bkg_exp:
    ...
    func_ExpLin_mZ.fitTo(mc_Wenu_mZ, RooFit.Range("MC_mZfit_range_SB1,MC_mZfit_range_SB2"));#ROOT.RooFit.Range("low,high")); # Fits background

    #Plotting background
    residue, chi2_bkg = root_plot(m_ee=m_ee, distribution=mc_Wenu_mZ, fit=func_ExpLin_mZ,
            mZmin=mZmin, mZmax=mZmax, title=f'Background for cut {fig_title}', save_dir = save_dir, DoF=DoF, nParam=nParam_bkg);
    #
    # =============================================================================
    ...
    DoF= np.array([h_mZWenu.GetBinContent(i) for i in range(NbinsZmass)])
    DoF = np.sum(DoF != 0)
    ...
    nParam = nParam_bkg + nParam_sig
    if plots:
        mc_ZeeWenu_mZ_resid, chi2_comb = root_plot2(m_ee=m_ee, distribution=mc_ZeeWenu_mZ, fit=func_SigBkg_mZ, fit2=func_ExpLin_mZ,
                  mZmin=mZmin, mZmax=mZmax, title=f'Bkg+Sig for cut {fig_title}', save_dir = save_dir, DoF = DoF, nParam=nParam);
        mc_ZeeWenu_mZ_resid, chi2_comb = root_plot(m_ee=m_ee, distribution=mc_ZeeWenu_mZ, fit=func_SigBkg_mZ,
                  mZmin=mZmin, mZmax=mZmax, title=f'Bkg+Sig comb for cut {fig_title}', save_dir = save_dir, DoF = DoF, nParam=nParam);
    ...
    bkg = ntotal.getVal()*f_bkg_mZ.getVal()
    sig = ntotal.getVal()*(1-f_bkg_mZ.getVal())
    ...
    #MC OS
    # exp
    #BL_sig = 66281 * (1-0.9346)#74747 * (1-0.4427)#41054
    # BL_sig = 98531 * (1-0.7509)#74747 * (1-0.4427)#41054
    #BL_bkg = 66281 * 0.9346
    ...
    ## Zee
    # BL_sig = 324912 * (1-0.9015)
    # BL_bkg = 324912 * 0.9015
    #
    ...
    return sig, bkg, f_bkg_mZ.getVal(), chi2_comb#BL_sig, BL_bkg, sig_ratio, bkg_ratio, chi2_comb#max_residue, ntotal.getVal(), nsig.getVal(), nbkg.getVal()return sigmaCB if CB else sigmaGA #sig_ratio, sigma_sig, bkg_ratio, sigma_bkg
    #else:
    #    return 0, 0, 0, 0, 0
