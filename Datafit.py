...
# -*- coding: utf-8 -*-
"""
Created on Wednesday 24 November
@Author: Mads Juhl Storr-Hansen
With a lot of inspiration from Sara Dahl Andersen
"""
...
from utils import timediftostr, Build_Folder_Structure, header, footer, print_col_names, h5ToDf, accuracy, HyperOpt_RandSearch, auc_eval, HeatMap_rand, mkdir
...
def GetATLASBkg(data):
    return ( ( data['muo1_charge']*data['muo2_charge'] > 0 ) & #same sign
             ( (data['muo1_pt']) > 10000 ) &
             ( (data['muo2_pt']) > 10000 ) &
             ((np.abs(data_get["muo1_eta"])<1.37) | ((np.abs(data_get["muo1_eta"])>1.52) & (np.abs(data_get["muo1_eta"])<2.7))) &
             ((np.abs(data_get["muo2_eta"])<1.37) | ((np.abs(data_get["muo2_eta"])>1.52) & (np.abs(data_get["muo2_eta"])<2.7))) &
...
#============================================================================
# LGBM dataset and parameters, and prediction
#============================================================================

log.info(f"Predicting")
t_start = time()
#Adding required variables
data['muo1_ptvarcone40_rel'] = (data['muo1_ptvarcone40'].div(data['muo1_pt'])).values.tolist()
data['muo2_ptvarcone40_rel'] = (data['muo2_ptvarcone40'].div(data['muo2_pt'])).values.tolist()

data['muo1_ptvarcone20_rel'] = (data['muo1_ptvarcone20'].div(data['muo1_pt'])).values.tolist()
data['muo2_ptvarcone20_rel'] = (data['muo2_ptvarcone20'].div(data['muo2_pt'])).values.tolist()
...
data["muo1_mIso_score"] = mIsoScore(lIsoGBM, data, 1, n_jobs = args.njobs)
data["muo1_mPid_score"] = mPidScore(lPidGBM, data, 1, n_jobs=args.njobs)
data["muo2_mIso_score"] = mIsoScore(lIsoGBM, data, 2, n_jobs = args.njobs)
data["muo2_mPid_score"] = mPidScore(lPidGBM, data, 2, n_jobs = args.njobs)
data["mm_score"] = ZmmScore(ZllGBM, data, n_jobs = args.njobs)
data["predLGBM"] = ZmmgScore(ZllgGBM, data, n_jobs = args.njobs)
...
print(data["pho_pIso_score"])

fig, ax = plt.subplots(figsize=(7,5))
ax.hist(data["mm_score"], bins = 100, range = (-10,15), histtype="step", label = "All data")
...
ax[0].hist(dataATLAS["Z_score"], bins = 100, histtype="step", range = (-20,20), label = "ATLAS");
ax[0].hist(dataLGBM["Z_score"], bins = 100, histtype="step", range = (-20,20), label = "LGBM");
ax[0].set(xlabel = "Z_score");
ax[0].legend()

ax[1].hist(dataATLAS["pho_pPid_score"], bins = 100, histtype="step", range = (-5,5), label = "ATLAS");
ax[1].hist(dataLGBM["pho_pPid_score"], bins = 100, histtype="step", range = (-5,5), label = "LGBM");
ax[1].set(xlabel = "pho_pPid_score");
ax[1].legend()

ax[2].hist(dataATLAS["pho_pIso_score"], bins = 100, histtype="step", range = (-5,5), label = "ATLAS");
ax[2].hist(dataLGBM["pho_pIso_score"], bins = 100, histtype="step", range = (-5,5), label = "LGBM");
ax[2].set(xlabel = "pho_pIso_score");
ax[2].legend()
