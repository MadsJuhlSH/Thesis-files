...
parser.add_argument('--outdir', action='store', default="output/SignalFiles/Hgamgam/", type=str,
...
def getTagsAndProbes(hf, event, process):
    ...
    pho = np.arange(0,len(hf['pho_et'][event]),1)

    #Maybe tight criteria?
...
def addPhotonVariables(hf, event, data_temp, phoNr, pho):
    ...
    data_temp[ 0, column_names.index( f'pho{phoNr}_ConversionType') ] = hf[ 'pho_ConversionType' ][ event][ pho ]
    ...
    try:
        data_temp[ 0, column_names.index( f'pho{phoNr}_core57cellsEnergyCorrection') ] = hf[ 'pho_core57cellsEnergyCorrection' ][ event][ pho ]
    except:
        data_temp[ 0, column_names.index( f'pho{phoNr}_core57cellsEnergyCorrection') ] = -999
    ...
    for i, event  in enumerate(np.arange(start,stop)):
        ...
        if npho>1:
            ...
            for iPhoprim, Phoprim in enumerate(sortPho):
                #print(f"IPhoPrim: {iPhoprim}")
                #print(f"PhoPrim: {Phoprim}")

                if hf["pho_et"][event][Phoprim]<9.5:
                    continue
                ...
                for iPhosecond, Phosecond in enumerate(phosecond):
                    """
                    At least two loose photons
                    two highest pt photons are the candidates - all other is background
                    Tag needs over 35 GeV, Probe needs over 25 GeV, and require both to have pdgId as a photon
                    """
                    if (iPhoprim!=0) and (iPhosecond!=0):
                        continue
                    if hf["pho_et"][event][Phosecond]<9.5:
                        continue
