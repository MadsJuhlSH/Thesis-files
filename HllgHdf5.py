...
This code is used for the signal selection of H->Z(->ll)gam
...
parser.add_argument('--outdir', action='store', default="output/SignalFiles/Hllgam/", type=str,
...
parser.add_argument('--phoOrigin', action='store', default=14, type=int,
                    help='Photon origin used to detirmine signal. (Default = 14 [Higgs])')
...
def combinedVariables(hf, event, lTag, lProbe, phoProbe, lType):
    """
...
def getTagsAndProbes(hf, event, process):
    ...
    return eTag, eProbe, mTag, mProbe, pProbe
...
def multiExtract(arguments):
    ...
    for i, event  in enumerate(np.arange(start,stop)):
        ...
        # Get tags and probes
        try:
            eTag, eProbe, mTag, mProbe, pProbe = getTagsAndProbes(hf,event, process)
        except: 
            eTag, eProbe, mTag, mProbe, pProbe = [],[],[],[],[]
        neTag, nmTag = len(eTag), len(mTag)
        print(f"There are {neTag} ele tag, {len(eProbe)} ele probe, {nmTag} muo tag, {len(mProbe)} muo probe and {len(pProbe)} pho probes.")
        # If there is at least one tag, 2 ele of same kind and 1 photon -> get values
        if ((neTag>0) & (len(eProbe)+neTag >1) & (len(pProbe)>0) ):
            # Event passed trigger cut - add to histogram
            histTrigPass[0,1] +=  1

            for iLTag, LTag in enumerate(eTag):
                # Get Probes
                lProbes = eProbe.copy()
                ...
                for iLProbe, LProbe in enumerate(lProbes):
                    for iPhoProbe, phoProbe in enumerate(phoProbes):
                        ...
                        invM, et, eta, phi, invMll = combinedVariables(hf, event, LTag, LProbe, phoProbe, "ele")
                        ...
                        data_temp[ 0, column_names.index( 'lep_type' ) ] = "ele"
        ...
        if ((nmTag>0) & (len(mProbe)+nmTag >1) & (len(pProbe)>0) ):
            # Event passed trigger cut - add to histogram
            histTrigPass[0,1] +=  1

            for iLTag, LTag in enumerate(mTag):
                # Get Probes
                lProbes = mProbe.copy()
                phoProbes = pProbe.copy()
                # Append subsecuent lepton tags to lepton probes (applies if there is more than one tag lepton)
                ...
                for iLProbe, LProbe in enumerate(lProbes):
                    for iPhoProbe, phoProbe in enumerate(phoProbes):
                        ...
                        if (hf['muo_truthPdgId'][event][LTag]*hf['muo_truthPdgId'][event][LProbe]>0): #same charge
                            ...
                            selection = signalSelection(hf, event, LTag, LProbe, phoProbe, histNtype,args.phoOrigin, process)
                        else:
                            selection = 2
                            histNtype[0,2] += 1
                        ...
                        if selection == 2:
                            continue
                        # Calculate varaibles of the electron pair and the photon
                        invM, et, eta, phi, invMll = combinedVariables(hf, event, LTag, LProbe, phoProbe, "muo")
                        data_temp[ 0, column_names.index( 'NvtxReco' ) ] = np.int(hf['NvtxReco'][event])
                        data_temp[ 0, column_names.index( 'correctedScaledAverageMu' ) ] = hf[ 'correctedScaledAverageMu' ][ event ]
                        data_temp[ 0, column_names.index( 'invM' ) ] = invM
                        data_temp[ 0, column_names.index( 'invMll' ) ] = invMll
                        ...
                        data_temp[ 0, column_names.index( 'type' ) ] = selection
                        data_temp[ 0, column_names.index( 'bkgtype') ] = bkgsel
                        data_temp[ 0, column_names.index( 'lep_type' ) ] = "muo"
...
def MassHistogram(HistVal, DatType, Orig, bkgtype,leptype, fname, title, xlabel, min, max, nbins):
    """
    Simply plots histogram and saves it.
    ...
    ax[0].hist(HistVal[leptype=="ele"][DatType==1],bins=np.linspace(min,max,nbins),histtype="step", label = "Signal")
    ax[0].hist(HistVal[leptype=="ele"][DatType == 0],bins=np.linspace(min,max,nbins),histtype="step", label = "Background")
    ax[1].hist(HistVal[leptype=="muo"][DatType==1],bins=np.linspace(min,max,nbins),histtype="step", label = "Signal")
    ax[1].hist(HistVal[leptype=="muo"][DatType == 0],bins=np.linspace(min,max,nbins),histtype="step", label = "Background")
    ax[0].set_title("invM of electrons")
    ax[1].set_title("invM of muons")
    plt.tight_layout()
    fig.savefig(args.outdir+"eleVSmuo")
...
All_masses, All_mll, All_SBT, All_orig, ALL_bkgtype, All_leptype = [], [], [], [], [], []
...
for path in args.paths:
    ...
    All_leptype = np.append(All_leptype,data[:,column_names.index("lep_type")])
